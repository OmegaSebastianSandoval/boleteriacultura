		...
		<div class="row mx-auto mt-3 px-2">
			<div class="col-12">
				<div class="alert alert-info mb-0 py-2 px-3" style="font-size:.78rem;">
					<i class="fas fa-hand-pointer"></i> <strong>Da clic en una mesa</strong> del mapa para ver su información, reserva
					e invitados asignados.
				</div>
			</div>
		</div>
		...
		<div class="row mx-auto mt-2 g-3 px-2">
			...
			<div class="col-6">
				<div style="background:#fff;border:1px solid #e2e8f0;border-radius:12px;padding:14px;height:100%;">
					...
					<div
						style="font-size:.72rem;font-weight:700;color:#94a3b8;letter-spacing:.08em;text-transform:uppercase;margin-bottom:10px;">
			...
			<div class="col-6">
				<div style="height:100%;display:flex;flex-direction:column;gap:10px;">
					...
					<div class="nameMesa" style="flex:1;transition:all .2s;">
						<div
							style="background:#f8fafc;border:1px dashed #cbd5e1;border-radius:12px;padding:24px;text-align:center;height:100%;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;">
							...
							<div style="font-size:.75rem;color:#cbd5e1;">Verás aquí los detalles de la mesa,<br>la reserva asignada y la
								lista de invitados.</div>
						</div>
					</div>
		...
		<div class="row mx-auto px-2 mt-2">
			<div class="col-12">
				<div class="alert alert-info mb-0 py-2 px-3" style="font-size:.75rem;">
					...
					<strong>Leyenda:</strong> Las mesas en <span style="color:#f59e0b;font-weight:600;">amarillo</span> tienen reserva
					asignada. Las <span style="color:#22c55e;font-weight:600;">verdes</span> están disponibles.
				</div>
			</div>
		</div>
	<?php endif; ?>
...
<?php } else { ?>
					...
					<div class="col-2 form-group">
						...
						<label class="input-group">
							<input type="datetime-local"
								value="<?= $this->content->ambiente_fecha_partido ? date('Y-m-d\TH:i', strtotime($this->content->ambiente_fecha_partido)) : ''; ?>"
								name="ambiente_fecha_partido" id="ambiente_fecha_partido" class="form-control">
						</label>
						<div class="help-block with-errors">Las reservas cierran automáticamente al arrancar el partido, o antes si no
							quedan mesas disponibles.</div>
					</div>
					<div class="col-2 form-group">
						<label for="ambiente_precio_silla" class="control-label">Precio silla (por defecto)</label>
						<label class="input-group">
							<input type="number"
								value="<?= ($this->content->ambiente_precio_silla) ? $this->content->ambiente_precio_silla : ''; ?>"
								name="ambiente_precio_silla" id="ambiente_precio_silla" class="form-control" min="0" step="1">
						</label>
						<div class="help-block with-errors">Precio sugerido al crear una silla nueva en este ambiente. No afecta
							sillas ya creadas.</div>
					</div>
					<div class="col-12">
						<?php if (!empty($_GET['saved'])): ?>
							<div id="toast-saved"
								style="display:flex;align-items:center;justify-content:center;gap:9px;background:#f0fdf4;color:#15803d;border:1px solid #bbf7d0;padding:13px 24px;border-radius:8px;font-size:0.95rem;font-weight:500;margin-top:4px;">
	...
	<div class="modal fade text-start" id="modalResumenAmbiente" tabindex="-1" aria-hidden="true">
		<div class="modal-dialog" style="max-width:95%;width:95%;" role="document">
			<div class="modal-content">
				...
				<div class="modal-body" id="resumenAmbienteCuerpo"
					style="background:var(--bg);padding:1rem;max-height:93vh;overflow-y:auto;">
			...
			<div class="modal-mesa-body">
				<div class="form-group">
					...
					<select id="input_tipo" class="form-control"
						onchange="actualizarDimensionesPorTipo(); actualizarCapacidadDefault(); actualizarVisibilidadPrecioSilla();">
				...
				<div class="form-row">
					<div class="form-group col">
						<label>Ancho</label>
						<input id="input_ancho" type="number" class="form-control" value="2" min="1"
							onchange="actualizarCapacidadDefault()">
					</div>
					<div class="form-group col">
						<label>Alto</label>
						<input id="input_alto" type="number" class="form-control" value="2" min="1"
							onchange="actualizarCapacidadDefault()">
					</div>
					<div class="form-group col">
				...
				<div class="form-group" id="grupo_precio_silla" style="display:none;">
					...
					<input id="input_precio" type="number" class="form-control" min="0" step="1"
						placeholder="<?php echo ($this->content->ambiente_precio_silla) ? $this->content->ambiente_precio_silla : ''; ?>">
	...
	<button class="btn-floating-add" id="btn-agregar-elemento" data-bs-toggle="tooltip" data-bs-placement="left"
	...
	<div class="container-fluid mb-3">
		<div class="content-dashboard">
			...
			<div id="scroll-top-bar-grid"
				style="overflow-x:auto; overflow-y:hidden; width:100%; height:18px; margin-bottom:2px;">
			</div>
...
<style>
	...
	<?php if (!empty($destacarMesaIds) && $modoValidacion === 'validacion'): ?>
		...
		/* Estilos específicos para resaltar las mesas/sillas consultadas.
			 La clase .mesa-destacada se asigna vía JS (ver renderElementos) en vez de generar
			 el selector con PHP aquí, que confundía al linter de CSS con un reguero de errores. */
		.elemento.mesa-destacada {
...
<script>
	...
	const destacarMesaIds = <?= json_encode(array_values($destacarMesaIds)) ?>; // IDs a resaltar en modo validación (ver .mesa-destacada)
	...
	function renderElementos () {
		elementos.forEach(el => {
			...
			const estadoDestacado = destacarMesaIds.includes(Number(el.mesa_id)) ? 'mesa-destacada' : '';
			div.className = `elemento ${el.mesa_tipo} ${el.mesa_estado || ''} ${estadoClase} ${estadoActivo} ${estadoDestacado}`;
